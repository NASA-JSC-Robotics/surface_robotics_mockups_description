...
class MockupStateManager(Node):
    def __init__(self):
        ...
        for index, mockup_config in enumerate(self.mockup_configs):
            ...
            subscriptions.append(
                self.create_subscription(Float64, self.prefix + mockup_config.topic_name, lambda msg, idx=index: self.position_cb(msg, idx), 10)
            )
    ...
    def load_mockup_configs(self):
        ...
        for topic in self.topic_names:
            self.get_logger().info('Loading: {}'.format(self.prefix + topic))
            ...
            initial_position = self.get_parameter("topics." + topic + ".initial_position").get_parameter_value().double_value
    ...
    def position_cb(self, msg: Float64, index):
        ...
        """
        self.get_logger().debug('This is the new position of {}: {}'.format(self.prefix + self.mockup_configs[index].joint_name,msg))
