...
class Hatch4040Manager(Node):
    def __init__(self):
        ...
        self.ext_rot_sub = self.create_subscription(
            Float64, self.prefix + "ext_wheel_position", self.ext_wheel_pos_cb, 10
        )
        self.ext_rot_h_sub = self.create_subscription(
            Float64, self.prefix + "ext_handle_position", self.ext_handle_pos_cb, 10
        )
        self.int_rot_sub = self.create_subscription(
            Float64, self.prefix + "int_wheel_position", self.int_wheel_pos_cb, 10
        )
        self.int_rot_h_sub = self.create_subscription(
            Float64, self.prefix + "int_handle_position", self.int_handle_pos_cb, 10
        )
        ...
        self._hatch_joint_states = {
            self.prefix + "hatch_outer_frame_face_joint": copy.deepcopy(self.hatch_position),
            self.prefix + "external_rotary_joint": copy.deepcopy(self.ext_wheel_position),
            self.prefix + "external_rotary_handle_joint": copy.deepcopy(self.ext_handle_position),
            self.prefix + "internal_rotary_joint": copy.deepcopy(self.int_wheel_position),
            self.prefix + "internal_rotary_handle_joint": copy.deepcopy(self.int_handle_position),
        }
    ...
    def hatch4040_joint_state_cb(self):
        ...
        msg.name = [
            self.prefix + "hatch_outer_frame_face_joint",
            self.prefix + "external_rotary_joint",
            self.prefix + "external_rotary_handle_joint",
            self.prefix + "internal_rotary_joint",
            self.prefix + "internal_rotary_handle_joint",
        ]
        with self.lock:
            msg.position = [
                self.hatch_position,
                self.ext_wheel_position,
                self.ext_handle_position,
                self.int_wheel_position,
                self.int_handle_position,
            ]  # fill with float64 list
    ...
    def hatch_pos_cb(self, msg: Float64):
        ...
        """
        self.get_logger().debug(f"This is the angle of the hatch joint: {msg}")
    ...
    def ext_wheel_pos_cb(self, msg: Float64):
        ...
        Args:
            msg (Float64): This message transmits the revolute joint angle (in radians)
        """
        self.get_logger().debug(f"This is the value of the external wheel joint: {msg}")
    ...
    def ext_handle_pos_cb(self, msg: Float64):
        ...
        Args:
            msg (Float64): This message transmits the revolute joint angle (in radians)
        """
        self.get_logger().debug(f"This is the value of the external handle joint: {msg}")
    ...
    def int_wheel_pos_cb(self, msg: Float64):
        ...
        Args:
            msg (Float64): This message transmits the revolute joint angle (in radians)
        """
        self.get_logger().debug(f"This is the angle of the internal wheel joint: {msg}")
